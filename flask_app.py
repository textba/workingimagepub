...
def api_generate_video():
    ...
    for name, w, h in selected:
        ...
        if intro_path:
            ...
            # Scale image to match output width, then pan vertically top->bottom when tall enough.
            scale = w / src_w
            scaled_h = int(src_h * scale)

            if scaled_h > h:
                scaled = src.resize((w, scaled_h), Image.Resampling.LANCZOS)
                full_arr = np.array(scaled)
                ...
                clip = VideoClip(make_scroll_frame, duration=duration).with_audio(audio)
            else:
